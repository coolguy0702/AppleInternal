...
class TestSysConfig(unittest.TestCase):
    ...
    def test_get_platform(self):
        ...
        dt_keys = [k for k in get_config_vars().keys() if k.endswith('_DEPLOYMENT_TARGET')]
        for k in dt_keys:
            del get_config_vars()[k]

        get_config_vars()['IPHONEOS_DEPLOYMENT_TARGET'] = '13.0'
        self.assertEqual(get_platform(), 'iphoneos-13.0')
    ...
    @unittest.skipIf(sys.platform == 'darwin' and sysconfig.get_config_var('TARGET_OS_EMBEDDED'),
                     'Test not compatible with embedded darwin')
    def test_get_config_h_filename(self):
        config_h = sysconfig.get_config_h_filename()
        self.assertTrue(os.path.isfile(config_h), config_h)

    def test_get_scheme_names(self):
        wanted = ('nt', 'nt_user', 'osx_framework_library', 'osx_framework_user',
    ...
    def test_osx_ext_suffix(self):
        suffix = sysconfig.get_config_var('EXT_SUFFIX')
        self.assertTrue(suffix.endswith('.so'), suffix)
