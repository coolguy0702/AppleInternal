...
current_module_files = dict()
@contextmanager
def module_file(modname):
    global FILENAME
    global NONEXISTENT_FILENAME
    spec = find_spec(modname)
    if isinstance(spec.loader, SourceLoader):
        filename = spec.origin
        yield filename
    elif modname in current_module_files:
        yield current_module_files[modname]
    else:
        ...
        with NamedTemporaryFile('w+', delete=False) as temp:
            ...
            global NONEXISTENT_FILENAME
            try:
                if modname == 'linecache':
                    FILENAME = temp.name
                    NONEXISTENT_FILENAME = temp.name + '.missing'
                current_module_files[modname] = temp.name
                yield temp.name
            finally:
                del current_module_files[modname]
                if modname == 'linecache':
                    FILENAME = linecache.__file__
                    NONEXISTENT_FILENAME = FILENAME + '.missing'
...
class LineCacheTests(unittest.TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.ctx = module_file('linecache')
        self.ctx.__enter__()
    ...
    def test_getline(self):
        ...
        for entry in MODULES:
            with module_file(entry) as filename:
