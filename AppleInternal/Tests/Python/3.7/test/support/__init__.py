...
def findfile(filename, subdir=None):
    ...
    if not os.path.exists(filename) and filename.endswith('.py'):
        import importlib.util
        from tempfile import NamedTemporaryFile
        modname = filename[:-3].replace(os.path.sep, '.')
        spec = importlib.util.find_spec(modname)
        if spec:
            source = spec.loader.get_source(modname)
            temp = NamedTemporaryFile(mode='w+', prefix=filename, delete=False)
            temp.write(source)
            temp.close()
            class Tempname(str):
                pass
            tempname = Tempname(temp.name)
