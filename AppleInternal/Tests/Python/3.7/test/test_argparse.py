...
class TestEncoding(TestCase):
    ...
    def _test_module_encoding(self, spec):
        from importlib.abc import SourceLoader
        if not isinstance(spec.loader, SourceLoader):
            # source loaders already return source as unicode, so this test
            # doesn't really make sense if the sources in files.
            raise unittest.SkipTest("non standard source loader")
        path = spec.origin
    ...
    def test_argparse_module_encoding(self):
        self._test_module_encoding(argparse.__spec__)

    def test_test_argparse_module_encoding(self):
        self._test_module_encoding(__spec__)
