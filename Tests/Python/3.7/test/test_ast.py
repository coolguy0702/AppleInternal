...
class ASTValidatorTests(unittest.TestCase):
    ...
    def iter_stdlib_sources(self, spec):
        import pkgutil
        from importlib.util import find_spec
        stdlib_loader = find_spec('os').loader
        for i in pkgutil.iter_modules():
            if i.ispkg or not i.module_finder.find_module('os'):
                continue
            spec = find_spec(i.name)
            source = spec.loader.get_source(i.name)
            yield spec.origin, source

    def test_stdlib_validates(self):

        for fn, source in self.iter_stdlib_sources(ast.__spec__):
            mod = ast.parse(source, fn)
            compile(mod, fn, "exec")

        for modname in ["test.test_grammar", "test.test_unpack_ex"]:
            spec = importlib.util.find_spec(modname)
            source = spec.loader.get_source(modname)
            mod = ast.parse(source, modname)
            compile(mod, spec.origin, "exec")
