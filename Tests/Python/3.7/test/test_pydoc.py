...
def run_pydoc(module_name, *args, **env):
    ...
    rc, out, err = assert_python_ok('-B', '-m', 'pydoc', *args, **env)
    return out.strip()
...
def get_pydoc_link(module):
    ...
    basedir = dirname(pydoc.__file__)
    doc = pydoc.TextDoc()
...
class PydocDocTest(unittest.TestCase):
    ...
    def test_synopsis_sourceless(self):
        ...
        if not isinstance(find_spec("os").loader, SourceLoader):
            raise unittest.SkipTest("nonstandard source loader")
        
        expected = os.__doc__.splitlines()[0]
        filename = os.__cached__
        if not os.path.exists(filename):
            raise unittest.SkipTest("pyc file is not installed")
...
class PydocImportTest(PydocBaseTest):
    ...
    def test_importfile(self):
        from importlib.abc import SourceLoader
        from importlib.util import find_spec
        if not isinstance(find_spec("pydoc").loader, SourceLoader):
            raise unittest.SkipTest("nonstandard source loader")
...
class PydocUrlHandlerTest(PydocBaseTest):
    ...
    def test_url_requests(self):
        ...
        with self.restrict_walk_packages():
            ...
            # another test that wants to load a source file by opening its path
            from importlib.abc import SourceLoader
            from importlib.util import find_spec
            if not isinstance(find_spec("string").loader, SourceLoader):
                raise unittest.SkipTest("nonstandard source loader")
