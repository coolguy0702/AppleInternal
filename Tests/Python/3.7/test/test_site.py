...
class ImportSideEffectTests(unittest.TestCase):
    ...
    def test_abs_paths(self):
        ...
        #see the comment in site.py about "don't mess with a PEP 302-supplied __file__"
        if (getattr(getattr(os, '__loader__', None), '__module__', None) not in
                ('_frozen_importlib', '_frozen_importlib_external')):
            raise unittest.SkipTest("nonstandard loader")
...
class StartupImportTests(unittest.TestCase):
    ...
    @unittest.skip("we install 3rd party modules like pyobjc that install .pth files which break this test")
